...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            batch_indices = torch.arange(batch_size).unsqueeze(0).expand(2, -1)
            critic_indices = torch.randint(0, num_critic_networks, (2, batch_size))
            sampled_qs = next_qs[critic_indices, batch_indices]
            next_qs = torch.min(sampled_qs, dim=0)[0]
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward[None, :] + self.discount * (~done[None, :]) * next_qs
